...
namespace __pyllars_internal{
    ...
    struct PtrWrapperBaseBase: public CommonBaseWrapper{
        ...
        static PyObject* _addr(PyObject* self, PyObject *args, Tptr &cobj){
          ...
          try{
            ...
            pyobj = toPyObject< typename  PtrTo<Tptr>::type, is_complete>( (typename PtrTo<Tptr>::type) &cobj, false, 1);
        ...
        inline static PyObject* addr(PyObject* self, PyObject *args){
            try{
                ...
                Tptr* obj = PtrTo<Tptr>::cast( (Tptr*) &((reinterpret_cast<PtrWrapperBaseBase*>(self))->_all_content._untyped_content));
                return PtrWrapperBaseBase::_addr<Tptr, is_complete>  (self, args, *obj);
            } catch(const char* const msg){
                PyErr_SetString( PyExc_RuntimeError, msg);
                return nullptr;
            }
        ...
        static int _init( PtrWrapperBaseBase *self, PyObject *args, PyObject *kwds, PyTypeObject* pytypeobj, const ssize_t max) {
            int status = -1;
    ...
    protected:
        ...
        public:
            ...
            static T* cast( T const * t){ (void)t ; return nullptr;}
        };
    ...
    PyObject* PtrWrapperBaseBase::parent_module = nullptr;
       ...
       template <typename T, bool is_array, const ssize_t array_size, typename E = void>
        ...
        PyObject* set_array_values( T* values, const ssize_t size, PyObject* from, PyObject* referenced){
            ...
            } else if( PyObject_TypeCheck(from, (&PythonClassWrapper<T*, true>::Type))){
                //TODO: check array size on from????
                for(int i = 0; i < size; ++i){
                   values[i] = ((T*) ((PythonClassWrapper<T*, true>*)from)->_all_content._untyped_content)[i];
                }
            }else if( PyObject_TypeCheck(from, (&PythonClassWrapper<T[], true>::Type))){
                //TODO: check array size on from????
                for(int i = 0; i < size; ++i){
            ...
            }else if( array_size > 0 && size > 0 && PyObject_TypeCheck(from, (&PythonClassWrapper<T[array_size], true>::Type))){
                if(array_size != size){
                    PyErr_SetString(PyExc_RuntimeError, "Mistmached array lengths");
                    return nullptr;
                }
                for(int i = 0; i < size; ++i){
                   values[i] = ((T*) ((PythonClassWrapper<T[array_size], true>*)from)->_all_content._untyped_content)[i];
                }
            } else {
        ...
        template< >
        PyObject* set_array_values<const char**, false, -1, void>( const char** values, const ssize_t size, PyObject* from, PyObject* referenced){
            if (!PyList_Check(referenced) || PyList_Size(referenced)!=size){
                PyErr_SetString(PyExc_RuntimeError, "Internal error setting array elements");
            }
            if( PyList_Check(from)){
