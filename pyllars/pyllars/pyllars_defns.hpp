...
namespace __pyllars_internal {
    ...
    //////////
    template<typename T, const ssize_t max = std::extent<T>::value-1, typename E = void>
    ...
    template<typename T, bool del_is_pub,  bool is_array, const ssize_t array_size, typename std::enable_if<std::is_assignable<T, T>::value>::type>
    PyObject *set_array_values(T *values, const ssize_t size, PyObject *from, PyObject *referenced) {
        ...
        if (PyList_Check(from)) {
        ...
        } else if (PythonClassWrapper<T *, del_is_pub>::checkType(from)) {
            //TODO: check array size on from????
            for (int i = 0; i < size; ++i) {
                values[i] = ((T *) ((PythonClassWrapper < T * , del_is_pub> *)
            ...
            for (int i = 0; i < size; ++i) {
                ...
                PythonClassWrapper<T[array_size], del_is_pub>::checkType(from)) {
            if (array_size != size) {
                PyErr_SetString(PyExc_RuntimeError, "Mistmached array lengths");
                return nullptr;
            }
            for (int i = 0; i < size; ++i) {
                values[i] = ((T *) ((PythonClassWrapper < T[array_size], del_is_pub> *)
                from)->_all_content._untyped_content)[i];
            }
        } else {
