...
namespace __pyllars_internal {
    ...
    template<typename T>
    struct ObjContainer<T, typename std::enable_if< std::is_copy_constructible<T>::value>::type> {
    ...
    struct ObjContainer<T, typename std::enable_if< !std::is_copy_constructible<T>::value>::type> {
        ...
        template< typename ...Args>
        void set_contents(Args&... args){
           throw "Attempt to copy data under non-copiable type";
        }
    ...
    struct ObjContainerProxy : ObjContainer<T> {
        ...
        ObjContainerProxy(Args ...args) :
            ...
            contained2(std::forward<Args>(args)...) {
        }
    ...
    template<typename T>
    struct ObjContainerProxy<T, const T> : ObjContainer<T> {

        ObjContainerProxy(const T& arg) :
                ObjContainer<T>(contained2, 0, false),
                contained2(arg) {
        }
