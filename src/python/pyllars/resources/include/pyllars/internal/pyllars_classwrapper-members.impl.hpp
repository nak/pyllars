...
namespace pyllars_internal {
    ...
    void
    Classes<T>::addAttribute(typename MemberPtr<FieldType>::member_t member) {
        ...
        Base::getTypeProxy()._member_getters[name] = MemberContainer<name, T_NoRef, FieldType>::get;
        if constexpr (!std::is_const<FieldType>::value) {
            Base::getTypeProxy()._member_setters[name] = MemberContainer<name, T_NoRef, FieldType>::set;
        }
    }
